...
go 
alter proc View_New_Application
@HRusername varchar(255),@title varchar(255)
as
declare @company varchar(255), @department varchar(255)
select @company=s.company, @department=s.department
...
go 
alter proc Accept_Or_Reject_Manager_Accepted
...
go
 alter proc View_Attendance_Between_Certain_Period
...
--5 alter a new project in my department with all of its information
go
alter proc alter_New_Project
...
go 
alter proc Change_Regular_Task_Manager
@MHRusername varchar(255), @regular_employee varchar(255),@project_name varchar(255),@taskName varchar(255)
as
declare @MHRcompany varchar(255), @MHRdepartment varchar(255)
select @MHRcompany=s.company, @MHRdepartment=s.department
...
go
alter proc View_Tasks_Manager_With_Certain_Conditions
...
--12 Review a task that I alterd in a certain project which has a state ‘Fixed’, and either accept or reject it. If I accept it, then its state would be ‘Closed’, otherwise it will be re-assigned to the same regular employee with state ‘Assigned’. The task should have now a new deadline.
go
